...
def read_data_csv(file_path="analytics-obfuscated-faked.csv"):
    ...
    df = pd.read_csv(file_path, encoding='ISO-8859-1', low_memory=False, names=columns)
    return df
...
def categorize_features(df, id_column="pid", threshold=100):
    """
    ...
    """
    cat_features = []
    columns_to_process = df.drop(id_column, axis=1).columns[:-1]  # Exclude id_column and the last column
    ...
    for col in columns_to_process:
        ...
        if n > threshold:
            ...
            print("keep ", col, " as categorical feature")
...
def process_pid_data(df, pid, date_col, agg_cols):
    ...
    date_range = pd.date_range(start=start_timestamp, end=end_timestamp, freq='H')
...
def combine_all_pids(df, date_col, agg_cols):
    ...
    combined_df = combined_df.fillna(0) # TODO maybe remove...
    return combined_df
...
def create_target_traffic_by_target_columns(df, target_columns):
    ...
    for hr in [1,4,8,12,24,72,168]:         # TODO dynamical value, as well dynamical for the database
        for target in target_columns:       # TODO save target_columns into the database
            df[f"{target}_next_{str(hr)}_hr"] = df.groupby("pid")[target].shift(-1*hr)
